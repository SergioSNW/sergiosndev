...
function EditComponent(props) {
    ...
    const currentColorValue = ourColors.filter(
      x => x.name == props.attributes.colorName
    )[0].color

    function handleColorChange(colorCode) {
      // from the hex value, we need to find the colorName
    ...
    <>
      <BlockControls>
        ...
        <ToolbarGroup>
          <ToolbarButton isPressed={props.attributes.size === "large"} 
            onClick={() => props.setAttributes({size: "large"})}>
            Large
          </ToolbarButton>
          <ToolbarButton isPressed={props.attributes.size === "medium"} 
            onClick={() => props.setAttributes({size: "medium"})}>
            Medium
          </ToolbarButton>
          <ToolbarButton isPressed={props.attributes.size === "small"} 
